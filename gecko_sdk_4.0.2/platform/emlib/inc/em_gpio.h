...
__STATIC_INLINE unsigned int GetDBGRegPinStatus(uint8_t pin)
{
  unsigned int val = BUS_RegBitRead(&(GPIO->DBGROUTEPEN), pin);
  return val;
}

__STATIC_INLINE void SetDBGRegPinStatus(uint8_t pos, uint8_t val)
{
  BUS_RegBitWrite(&(GPIO->DBGROUTEPEN),pos, val);
}
