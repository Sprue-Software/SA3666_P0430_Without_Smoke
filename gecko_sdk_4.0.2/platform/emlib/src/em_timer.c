...
void TIMER_Init(TIMER_TypeDef *timer, const TIMER_Init_TypeDef *init) {
	...
	timer->CFG = ((uint32_t) init->prescale << _TIMER_CFG_PRESC_SHIFT)
			...
			| (init->quadModeX4 ? TIMER_CFG_QDM_X4 : 0) | (init->oneShot ? TIMER_CFG_OSMEN : 0)
			| (init->sync ? TIMER_CFG_SYNC : 0) | (init->ati ? TIMER_CFG_ATI : 0);
...
void TIMER_InitCC(TIMER_TypeDef *timer, unsigned int ch, const TIMER_InitCC_TypeDef *init) {
...
#endif
	timer->CC[ch].CFG = ((uint32_t) init->mode << _TIMER_CC_CFG_MODE_SHIFT)
			| (init->filter ? TIMER_CC_CFG_FILT_ENABLE : 0) | (init->coist ? TIMER_CC_CFG_COIST : 0)
	...
	timer->CC[ch].CTRL = ((uint32_t) init->eventCtrl << _TIMER_CC_CTRL_ICEVCTRL_SHIFT)
...
void TIMER_InitDTI(TIMER_TypeDef *timer, const TIMER_InitDTI_TypeDef *init) {
	...
	timer->DTTIMECFG = ((uint32_t) init->prescale << _TIMER_DTTIMECFG_DTPRESC_SHIFT)
			| ((uint32_t) init->riseTime << _TIMER_DTTIMECFG_DTRISET_SHIFT)
			| ((uint32_t) init->fallTime << _TIMER_DTTIMECFG_DTFALLT_SHIFT);

	timer->DTFCFG = (init->enableFaultSourceCoreLockup ? TIMER_DTFCFG_DTLOCKUPFEN : 0)
...
void TIMER_SyncWait(TIMER_TypeDef *timer) {
	while (((timer->EN & TIMER_EN_EN) != 0U) && ((timer->STATUS & TIMER_STATUS_SYNCBUSY) != 0U)) {
